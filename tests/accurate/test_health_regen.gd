...
func test_initial_values():
	...
	assert_eq(health_regen.regen_pause_length, 0.0, "Regen pause length is 0.")
	assert_eq(health_regen.regen_pause_when_damaged, true, "Regen pauses when health is damaged.")
	assert_eq(health_regen.regen_pause_time_left, 0.0, "No regen pause time.")
...
func test_value_clamping():
	health_regen.regen_pause_time_left = -1.0
	assert_eq(health_regen.regen_pause_time_left, 0.0, "Pause_time_left can't be negative.")
	health_regen.regen_pause_time_left = 2.0
	assert_eq(health_regen.regen_pause_time_left, health_regen.regen_pause_length, "Can't set pause_time_left above pause_length.")

func test_regen_pause_time_left():
	health_regen.regen_pause_length = 1.0
	health_regen.pause_regen()
	assert_eq(health_regen.regen_pause_time_left, health_regen.regen_pause_length, "Paused for 1 second.")
...
func test_get_time_since_pause():
	...
	assert_eq(health_regen.get_time_since_pause(), 1.0, "A second since pause.")
...
func test_regen_pause_length():
	health_regen.regen_in__process = false
	health_regen.regen_per_second = 60.0
	health_regen.regen_pause_length = 2.0
	health.damage(60)
	assert_eq(health_regen.regen_pause_time_left, 2.0, "Regen is paused for 2 seconds.")
	health_regen.simulate_regen(1.5)
	assert_eq(health.health, 40.0, "Health didn't change.")
	assert_eq(health_regen.regen_pause_time_left, 0.5, "Regen is paused for 0.5 seconds.")
	health_regen.simulate_regen(1.0)
	assert_eq(health.health, 70.0, "Half regenerated to 70.")
	assert_eq(health_regen.regen_pause_time_left, 0.0, "Regen is not paused since 0.5 seconds.")

func test_regen_pause_when_damaged():
	health_regen.regen_in__process = false
	health_regen.regen_per_second = 40.0
	health_regen.regen_pause_length = 100.0
	health_regen.regen_pause_when_damaged = false
	health.damage(40)
	assert_eq(health_regen.regen_pause_time_left, 0.0, "Regen is not paused.")
	health_regen.simulate_regen(1.0)
	assert_eq(health.health, health.max_health, "Fully regenerated.")
